...
const UserProfile = () => {
  ...
  // Para jugar un poco mas para las imagenes
  ...
  // Nos devuelve un array con todas nuestras ordenes
  const { data, isError, isLoading } = useQuery({
    queryKey: ["orders"],
    queryFn: myOrders,
  });

  // Obtenemos un solo user, esto con el fin de que cuando se actualiza los datos del usuario no se
  // tenga que salir e iniciar sesión de nuevo para poder ver esos cambios, ya que esos datos se
  // obtienen mediante el token.
  const { data: user } = useQuery({
    queryKey: ["users", id],
    queryFn: () => getUser(id),
  });

  // Nos trae todos los datos del usuario y los coloca en el estado, para que cuando se habrá la
  // ventana de editar, se muestren los datos del usuario ya cargados
  useEffect(() => {
    if (user) {
      setStateName(user.name);
      setStateLastName(user.last_name);
      setImage(user.avatar);
    }
  }, [user]);

  // POST para editar el perfil de usuario
          ...
          <div className="p-11">
            <form onSubmit={handleSubmit}>
              ...
              <div className="p-3">
                ...
                <input
                  value={stateLastName}
                  onChange={(e) => setStateLastName(e.target.value)}
                  type="text"
                  className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
